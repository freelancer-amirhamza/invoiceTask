...
const InvoiceModal = ({
  ...
  const initialData = useMemo(() => {
    ...
    const itemRows = items.map((item) => ({
      invoice_no: today+invoiceInfo?.invoiceNumber,
      customer: invoiceInfo.formData.customer,
      email: invoiceInfo.formData.email,
      company: invoiceInfo.formData.company,
      address: invoiceInfo.formData.address,
      phone: invoiceInfo.formData.phone,
      id: invoiceInfo.formData.id,
      taxRate: invoiceInfo.taxRate,
      discountRate: invoiceInfo.discountRate.toFixed(2),
      product: item.name,
      quantity: item.qty,
      subtotal: invoiceInfo.subtotal.toFixed(2),
      total: invoiceInfo.total.toFixed(2),
    }));
  ...
  const addNextInvoiceHandler = async() => {
    try {
      // Combine invoiceInfo and items into a single object
      const dataToSend = {
        ...invoiceInfo,
        items,
      };
      console.log(dataToSend, "true")

      // Send the POST request to the server
      const response = await axios.post('http://localhost:5000/invoice/add', dataToSend);

      // Handle the response as needed
      console.log('Invoice added successfully:', response.data);

      // Proceed to add the next invoice
      onAddNextInvoice();
      setIsOpen(false);
    } catch (error) {
      // Handle any errors
            ...
            <div className="my-8 inline-block w-full max-w-4xl transform overflow-hidden rounded-lg bg-white text-left align-middle shadow-xl transition-all">
              <div className="p-8 md:px-16 md:pt-16 md:pb-8 " id="print">
                ...
                <h1 className="text-center w-full  flex justify-end md:text-sm text-xs mt-2 font-semibold text-gray-600">
                  Transport Date: {invoiceInfo?.formData?.trans_date}
                </h1>
                </div>
                ...
                <div className="mt-6 border-dotted border-2 md:p-12 rounded-md p-8">
                  <div className="mb-4 grid grid-cols-2">
                    ...
                    <span className="font-semibold " >{invoiceInfo?.formData?.customer}</span>
                    </div>
                    <div className="flex flex-col py-2 ">
                    <label className="font-bold">Invoice No:</label>
                    <span className="font-semibold " >{today+invoiceInfo?.invoiceNumber}</span>
                    </div>
                    <div className="flex flex-col py-2 ">
                    <label className="font-bold">Email:</label>
                    <span className="font-semibold " >{invoiceInfo?.formData?.email}</span>
                    </div>
                    <div className="flex flex-col py-2 ">
                    <label className="font-bold">Phone:</label>
                    <span className="font-semibold " >{invoiceInfo?.formData?.phone}</span>
                    </div>
                    <div className="flex flex-col py-2 ">
                    <label className="font-bold">Address:</label>
                    <span className="font-semibold " >{invoiceInfo?.formData?.address}</span>
                    </div>
                    <div className="flex flex-col py-2 ">
                    <label className="font-bold">Company Name:</label>
                    <span className="font-semibold " >{invoiceInfo?.formData?.company}</span>
                    </div>
